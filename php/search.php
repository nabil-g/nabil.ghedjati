<?php
  ...
  if (isset($_POST['nomauteur']) && !empty($_POST['nomauteur'])) {
    ...
    $req = $bdd_cv->prepare('SELECT ID, nom, mail, message, DATE_FORMAT(date, \'%d/%m/%Y \') AS date_fr, time FROM messages WHERE nom LIKE ?'); // LIKE : recherche dynamique
    ...
    while ($donnees = $req->fetch()) {
      echo "<tr><td>" . $donnees['ID']."</td><td>". $donnees['nom'] . "</td><td>" . $donnees['mail'] . "</td><td>" . $donnees['date_fr'] ."</td><td>".$donnees['time']. "</td><td>" . $donnees['message'] . "</td></tr>" ;
    }
